...
def get_features_labels(data, corpus, dictionary, verbose):
    ...
    for doc_idx in xrange(num_docs):
        ...
        if verbose:
            print 'get_features_labels: extracted %d samples from document %d of %d' % (
                len(labels), doc_idx + 1, num_docs)
...
def get_keywordclassifier(preload, classifier_type):
    ...
    print 'Number of features = %d, Number of training samples = %d, Number of test samples %d' % (
        train_XY['features'].shape[1], train_XY['features'].shape[0], test_XY['features'].shape[0])
    print 'In-sample cross-validated accuracy: %.4f, Out-of-sample cross-validated accuracy: %.4f, Chance: 0.5' % (
...
def extract_keywords(text, keyword_classifier, top_k, preload):
    ...
    sorted_keywords = [(i[0],i[1]) for i in sorted(enumerate(predicted_prob[:, this_column]), key=lambda x: x[1], reverse=True)]
